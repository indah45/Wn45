...
wN = CBW.LINE()
#wN.login(qr=True)
wN.login(token='isi tokenmu')#chery
wN.loginResult()
...
Bots=[mid]
owner=["isi mid mu",mid]
admin=["isi mid mu",mid]
baby=["isi mid rekan mu"]#chery/barby/ranita
...
#        try:
#          wN.like(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],likeType=1002)
#          wN.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"👉ąµţ๏ℓɨЌ€ By✰ tɛǟʍ ċʏɮɛʀ-ǟʀʍʏ ɮօt ✰😊\n\n☆º°˚˚☆ tɛǟʍ ċʏɮɛʀ-ǟʀʍʏ ɮօt ✰°˚˚☆（＾ω＾）\nąµţ๏ℓɨЌ€ by CBW ⭐👈 »»» http://line.me/ti/p/GkwfNjoPDH «««")
#          print "Like"
#        except:
#          pass
#      else:
...
#                print "autoJoin is Off"
        ...
        if op.type == 19:
            if op.param3 in baby:
                 wN.kickoutFromGroup(op.param1,[op.param2])
                 wN.inviteIntoGroup(op.param1,baby)
                 wN.inviteIntoGroup(op.param1,[op.param3])
            else:
                pass
        if op.type == 19:
 ...
 #                    contact = wN.getContact(msg.from_)
 #                    cName = contact.displayName
 #                    balas = ["Dont Tag Me!! Im Busy, ",cName + " Ngapain Ngetag?, ",cName + " Nggak Usah Tag-Tag! Kalo Penting Langsung Pc Aja, ", "-_-, ","CBW lagi off, ", cName + " Kenapa Tag saya?, ","SPAM PC aja, " + cName, "Jangan Suka Tag gua, " + cName, "Kamu siapa, " + cName + "?", "Ada Perlu apa, " + cName + "?","Tag doang tidak perlu., "]
...
#========================== FOR COMMAND BOT STARTING =============================#
 ...
 #                       wN.sendText(msg.to,"Protection Cancel already Off")
                ...
                msg.contentMetadata = {'mid': "uc301fa8f0962f52b1f2d83dc251589cb"}
                wN.sendMessage(msg)
                wN.sendText(msg.to,'❂➣ Ini perwNnalkan FOUNDER NS3 Yang manis kalem  cakep,,unyu  kkkk  􀜁􀄯􏿿')
            ...
            if msg.contentType == 13:
               if wait["wblack"] == True:
                    if msg.contentMetadata["mid"] in wait["commentBlack"]:
                        ...
                        wait["commentBlack"][msg.contentMetadata["mid"]] = True
                        wait["wblack"] = False
                        wN.sendText(msg.to,"decided not to comment")
#--------------------------------------------------------
            elif msg.text is None:
                return
#--------------------------------------------------------
                ...
                if msg.from_ in owner:
                    ...
                    if gid == "":
                        ...
                        try:
                            ...
                            wN.sendText(msg.to,"succes di invite boss, silahkan masuk...!!")
                        except:
                            wN.sendText(msg.to,"Mungkin saya tidak di dalaam grup itu")

            elif "kabur all" in msg.text:
                if msg.from_ in owner:
                    if msg.toType == 2:
                        ginfo = wN.getGroup(msg.to)
                        try:
...
#                    wN.sendMessage(msg)
                ...
                if msg.from_ in owner:
                    wN.sendText(msg.to,"bot masih aktif Bebz...!!!")
#=============================================
#        if op.type == 17:
